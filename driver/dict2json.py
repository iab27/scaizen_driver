...
while 1:
	...
	servidor_driver_ucl = {
	    ...
	    "rtd":{
	        ...
	        "data": {
	            "temperatura": str((temperaturaTRrgl.registers[0]/100)+(temperaturaTRprm.registers[0]/100)),
	...
	try:
		id_process = broker.send_message_to_websocket_server(manager, json_txt)
		logging.info(id_process)
		if id_process%30 == 0:
			logging.info('clear')
			manager.reapall()
	except:
	...
	#Esto lo agregué
	#r=requests.post(url=API_ENDPOINT,json=json_txt)
	#print(tarea_id)
	#print(r.json)
	# Wait for 5 seconds
	time.sleep(0.5)
